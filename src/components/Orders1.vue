...
export default {
  ...
  methods: {
    ...
    pathVertices () {
      ...
      if (idx > 0) {
        const pt = [marginalXRate, vs[idx - 1][1]]
        vs.splice(idx, 0, pt)
      }
      return vs
    },
    ammPathVertices () {
      ...
      if (this.ammIsMandatory) {
        const y = this.amm.b2 - this.amm.b1 * this.minX
        return [[marginalXRate, 0], [this.minX, y]]
      } else {
    ...
    moveVertex (event) {
      ...
      if (this.draggingVertex % 2 === 0) {
        ...
        let pi = ptSvg[0]
        const minPi = orderIdx < this.orders.length - 1 ? this.orders[orderIdx + 1].pi : 0
